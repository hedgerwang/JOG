...
/**
 * This Class implements the actual scrolling logics for scrollable area.
 */
JX.install('MScrollAreaScroller', {
  /**
   ...
   *        setting.
   */
  construct : function(handler, opt_options) {
    // Ensure that these functions are always called as instance's methods.
    this._canAnimate = JX.bind(this, this._canAnimate);

    var options = opt_options || {};
    this._usePaging = options.snap;
    ...
    this._animator = new JX.MAnimator();
  },
  ...
  members: {
    ...
    _SPEED_FACTOR: (/android/gi).test(navigator.appVersion) ? 1 : 0.2,
    ...
    doTouchStart: function(jxEvent) {
      if (this.scrolling) {
        this._animator.stop();
        this.scrolling = false;
      }

      this._moveCoordStart = JX.MTouchHelper.getTouchPageCoord(jxEvent);
    ...
    doTouchMove: function(jxEvent) {
      if (!this._moveCoordStart || this._skipTouchMove) {
        return;
      }

      var moveCoord = JX.MTouchHelper.getTouchPageCoord(jxEvent);
      var now = JX.now();
    ...
    doTouchEnd: function(jxEvent) {
      ...
      var timeDelta = JX.now() - this._moveTimePrevious;
    ...
    _createStepFunction : function(p0, p1, p2, callback) {
      ...
      return JX.bind(this, function(percent, animating, timestamp) {
        var value;
        if (bounceDelta === 0) {
